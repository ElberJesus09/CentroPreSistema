...
@section('content')
    ...
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-xl font-semibold text-neutral-900">Nueva sede</h1>
    </div>

    <form
        method="post"
        action="{{ route('academic-cycles.campuses.store') }}"
        class="max-w-3xl space-y-6 rounded-lg border border-neutral-200 bg-white p-6 shadow-sm"
    >
        @csrf
        <div class="grid gap-4 sm:grid-cols-2">
            <x-input label="Nombre" name="name" :value="old('name')" />
            <div class="space-y-1">
