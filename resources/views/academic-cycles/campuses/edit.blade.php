...
@section('content')
    ...
    <div class="mb-6">
        <a href="{{ route('academic-cycles.campuses.index') }}" class="text-sm font-medium text-brand hover:underline">Volver al listado</a>
        <h1 class="mt-2 text-xl font-semibold text-neutral-900">Editar sede</h1>
    </div>

    <form
        method="post"
        action="{{ route('academic-cycles.campuses.update', $campus) }}"
        class="max-w-3xl space-y-6 rounded-lg border border-neutral-200 bg-white p-6 shadow-sm"
    >
        @csrf
        @method('PUT')
        <div class="grid gap-4 sm:grid-cols-2">
            <x-input label="Nombre" name="name" :value="old('name', $campus->name)" />
            <div class="space-y-1">
                ...
                >
                    <option value="1" @selected((string) old('status', $campus->status ? '1' : '0') === '1')>Activo</option>
                    <option value="0" @selected((string) old('status', $campus->status ? '1' : '0') === '0')>Inactivo</option>
                </select>
                @error('status')
            ...
            <div class="space-y-1 sm:col-span-2">
                <label for="address" class="block text-sm font-medium text-neutral-800">Direccion</label>
                <input
                    id="address"
                    name="address"
                    type="text"
        ...
        <div class="flex gap-3">
            ...
            <a
                href="{{ route('academic-cycles.campuses.index') }}"
