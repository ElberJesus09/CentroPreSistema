...
@section('title', 'Editar ciclo | '.config('app.name'))
...
@section('content')
    ...
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-xl font-semibold text-neutral-900">Editar ciclo</h1>
    </div>

    <form
        method="post"
        action="{{ route('academic-cycles.cycles.update', $cycle) }}"
        class="max-w-3xl space-y-6 rounded-lg border border-neutral-200 bg-white p-6 shadow-sm"
    >
        @csrf
        @method('PUT')
        <div class="grid gap-4 sm:grid-cols-2">
            <x-input label="Nombre (max 20)" name="name" :value="old('name', $cycle->name)" />
            <div class="space-y-1">
                ...
                >
                    <option value="1" @selected((string) old('status', $cycle->status ? '1' : '0') === '1')>Activo</option>
                    <option value="0" @selected((string) old('status', $cycle->status ? '1' : '0') === '0')>Inactivo</option>
                </select>
                @error('status')
                    <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <x-input label="Fecha inicio" name="start_date" type="date" :value="old('start_date', $cycle->start_date?->format('Y-m-d'))" />
            <x-input label="Fecha fin" name="end_date" type="date" :value="old('end_date', $cycle->end_date?->format('Y-m-d'))" />
        </div>
