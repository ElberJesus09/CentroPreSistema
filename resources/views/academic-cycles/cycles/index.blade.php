...
@section('content')
    ...
    <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        @can('create', \App\Models\AcademicCycle::class)
            <a
                href="{{ route('academic-cycles.cycles.create') }}"
        ...
        <tbody class="divide-y divide-neutral-100 text-neutral-800">
            @forelse ($cycles as $row)
                <tr class="hover:bg-neutral-50/80">
                    ...
                    <td class="whitespace-nowrap px-4 py-3">{{ $row->start_date?->format('Y-m-d') }}</td>
                    <td class="whitespace-nowrap px-4 py-3">{{ $row->end_date?->format('Y-m-d') }}</td>
                    ...
                    >
                        @can('update', $row)
                            <a href="{{ route('academic-cycles.cycles.edit', $row) }}" class="mr-2 text-sm font-medium text-brand hover:underline">
                                Editar
                            </a>
                        @endcan
                        @can('delete', $row)
                            <button
                                ...
                                onclick="document.getElementById('cycle-delete-{{ $row->id }}').showModal()"
                            >
                                Eliminar
                            </button>
                            <x-modal
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-4 py-8 text-center text-sm text-neutral-500">No hay ciclos registrados.</td>
                </tr>
            @endforelse
        </tbody>
    </x-table.shell>
